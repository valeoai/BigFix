...
from bigfix.network.transformer import Transformer
from bigfix.network.autoencoder import AutoEncoder
from bigfix.sampler.confidence_sampler import ConfidenceSampler
from bigfix.sampler.halton_sampler import HaltonSampler
...
class MaskGIT(Trainer):
    ...
    def get_resample_kwargs(self, code, y, drop_label):
        ...
        if method == "self_sample":
            # Sample from the EMA copy, matching what bigfix/sampler/confidence_sampler.py uses at
            # inference, and so this doesn't perturb the model that is about to be trained on.
            model = self.ema if self.args.use_ema else self.vit
            return {"model": model, "model_kwargs": {"y": y, "drop_label": drop_label}}
    ...
    def get_network(self, archi):
        ...
        if archi == "vit":
            ...
            model = Transformer(
                input_size=self.input_size, nclass=self.args.nb_class,
            ...
            learnable = True if self.args.vqgan_folder != "" else False
            model = AutoEncoder(num_levels=24, learnable=learnable, f_factor=self.args.f_factor)
            return model
    ...
    def train_one_epoch(self, log_iter=2_500, ema_update=1):
        ...
        for i_bar, data in enumerate(bar):
            ...
            resample_kwargs = self.get_resample_kwargs(code, y, drop_label) if self.args.p_resample > 0 else None
            masked_code, mask, loss_ign = get_mask_code(
                code, value=self.args.mask_value, codebook_size=self.args.codebook_size, mode=self.args.sched_mode,
                p_resample=self.args.p_resample, resample_method=self.args.resample_method,
                resample_p_schedule=self.args.resample_p_schedule, resample_kwargs=resample_kwargs
            )
    ...
    def fit(self, metrics_eval=10):
        ...
        for e in range(self.args.global_epoch, self.args.epoch+1):
            ...
            if e % metrics_eval == 0 and e > 0:
                m = self.eval(sampler=eval_sampler, num_images=50_000, save_exemple=False, compute_pr=False,
