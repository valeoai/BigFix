...
from bigfix.network.ema import EMA

from bigfix.metrics.sample_and_eval import SampleAndEval
...
class Trainer(object):
    ...
    def __init__(self, args):
        """ Initializes the Trainer class.
         :param
            args: Configuration object containing various hyperparameters and settings, see bigfix/main.py
        """
        self.args = args
    ...
    def transformer_size(self, size):
        ...
        if size == "tiny":
            ...
            hidden_dim, depth, heads = 1536, 32, 24
        else:
            hidden_dim, depth, heads = 768, 12, 12
    ...
    def log_add_img(self, names, img, iteration):
        ...
        img = vutils.make_grid(img, nrow=min(10, img.size(0)), padding=2, normalize=False)
    ...
    def adapt_learning_rate(self):
        ...
        #     self.optim.param_groups[0]['lr'] = max(self.args.lr * 0.5 * , self.args.lr * 0.01)
        else:
            progress = (self.args.iter - self.args.warm_up) / (self.args.max_iter - self.args.warm_up)
            progress = min(max(progress, 0.0), 1.0)
            new_lr = self.args.lr * 0.5 * (1 + math.cos(math.pi * progress))
            self.optim.param_groups[0]['lr'] = max(new_lr, self.args.lr * 0.01)
    ...
    def eval(self, sampler, num_images=50_000, save_exemple=True,
        ...
        if os.path.exists("./saved_networks/ImageNet_256_train_stats.pt") and self.args.data.startswith("imagenet"):
            if self.args.is_master:
                print("Use pre-computed ImageNet stats")
            use_precomputed_feat = "./saved_networks/ImageNet_256_train_stats.pt"
        else:
        ...
        self.sae = SampleAndEval(
            ...
            num_images=num_images, mode=mode, compute_manifold=compute_pr, use_precomputed_feat=use_precomputed_feat
        )
