...
#   python -m bigfix.main --mode "cls-to-img" ...
#   torchrun --standalone --nnodes=1 --nproc_per_node=gpu -m bigfix.main --mode "txt-to-img" ...
import os
import random
...
def main(args):
    ...
    if args.mode == "cls-to-img":
        from bigfix.trainer.cls_trainer import MaskGIT
    ...
    if args.test_only:
        ...
        maskgit.eval(sampler=eval_sampler, num_images=50_000, save_exemple=False, compute_pr=False,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--vq-type",       type=str,   default="vqgan-llama", help="llamagen|static")
    ...
    parser.add_argument("--proj",          type=int, default=1,              help="dropout in the transformer")
    ...
    parser.add_argument("--log-iter",      type=int,   default=2500,         help="generate new sample every ")
    # Learning
    ...
    parser.add_argument("--p-resample",    type=float, default=0.0,          help="resampling")
    parser.add_argument("--resample-method", type=str, default="shuffle",    help="how to corrupt resampled context tokens: shuffle|local_swap|freq|codebook_nn|self_sample")
    parser.add_argument("--resample-p-schedule", action='store_true',        help="scale p-resample by (1 - r) so early/low-context steps get corrupted more, like a real sampler's decaying noise")
    parser.add_argument("--swap-radius",   type=int,   default=1,            help="neighbourhood radius for resample-method=local_swap")
    parser.add_argument("--knn-k",         type=int,   default=8,            help="number of neighbours for resample-method=codebook_nn")
    parser.add_argument("--extra-cond",    action='store_true',              help="extra conditioning, e.g., reward")
    parser.add_argument("--compute-reward-on-the-fly", action='store_true',  help="compute CLIP/PickaPick/ImageReward during txt training")
    parser.add_argument("--drop-reward",    type=float, default=0.5,          help="drop rate for reward CFG")
