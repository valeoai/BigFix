...
from bigfix.metrics.inception_metrics import MultiInceptionMetrics


class SampleAndEval:
    def __init__(self, device, is_master, nb_gpus, num_images=50_000, num_classes=1_000, compute_manifold=True, mode="c2i", use_precomputed_feat=""):
        super().__init__()
        self.inception_metrics = MultiInceptionMetrics(
            device=device, compute_manifold=compute_manifold, num_classes=num_classes,
            num_inception_chunks=10, manifold_k=3, model="inception", use_precomputed_feat=use_precomputed_feat)
